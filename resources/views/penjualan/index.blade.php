    ...
    <div class="container py-4">
        ...
        @if (session('errors'))
            <div class="alert alert-danger alert-dismissible fade show rounded-4 border-0 shadow-sm mb-4" role="alert">
                <strong>⚠️ Ups! Ada Masalah:</strong> {{ session('errors') }}
        ...
        <div class="card bg-gradient-blue text-white shadow-sm mb-4 rounded-4 border-0">
            <div class="card-body p-4 text-center text-md-start d-flex flex-column flex-md-row align-items-center justify-content-between">
                ...
                <div class="mt-3 mt-md-0">
                    <a href="{{ route('penjualan.create') }}" class="btn btn-light text-primary fw-bold px-4 py-2 rounded-pill shadow-sm">
                        ✨ Transaksi Baru
                    </a>
                </div>
            </div>
                            ...
                            @forelse($sales as $sale)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if(strtolower($sale->metode_pembayaran) == 'cash' || strtolower($sale->metode_pembayaran) == 'tunai')
                                    ...
                                    <td class="text-center">
                                        @if(strtolower($sale->status) == 'lunas' || strtolower($sale->status) == 'completed' || strtolower($sale->status) == 'sukses')
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="{{ route('penjualan.show', $sale) }}" class="btn btn-sm btn-outline-info rounded-pill px-3 fw-medium">
                                                👁️ Detail
                                            </a>

                                            @can('view', $sale)
                                                <a href="{{ route('penjualan.edit', $sale) }}" class="btn btn-sm btn-outline-warning rounded-pill px-3 fw-medium">
                                                    ✏️ Edit
                                                </a>
                                            @endcan
